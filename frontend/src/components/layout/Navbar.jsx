...
function Navbar() {
  ...
  return (
    <nav className="bg-white shadow-sm">
        <div className="max-w-7xl mx-auto px-4 py-3 flex item justify-between ">
            ...
            <ul className="flex items-center gap-1 ">
               { menuItems.map((item) => (
                   <li key = {item.label}>
                    <a href={item.href} className="bg-white px-6 py-2 rounded-full text-sm text-gary-600 hover:bg-gray-100 font-bold hover:text-blue-300"> {item.label} </a>
                   </li>
                ))}
            </ul>
