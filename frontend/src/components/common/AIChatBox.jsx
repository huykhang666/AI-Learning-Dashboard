...
const AIChatBox = ({ onClose, sessionId, courseDetailApi, aiApi }) => {
    const [messages, setMessages] = useState([
        { role: 'ai', content: 'Chào Bạn! Mình đã sẵn sàng hỗ trợ bạn bài học này 🎓' }
    ]);
    ...
    const handleSendMessage = async () => {
        ...
        try {
            ...
            setMessages(prev => [
                ...
                {
                    role: 'ai',
                    content: res?.answer || "AI không trả lời"
                }
            ]);
            ...
            setMessages(prev => [
                ...prev,
                { role: 'ai', content: 'Lỗi rồi ơi, check Python AI!' }
            ]);
        } finally {
            setLoading(false);
        }
            ...
            <div ref={scrollRef} className="flex-1 overflow-y-auto p-6 space-y-6 bg-slate-50/30 custom-scrollbar">
                {messages.map((msg, idx) => (
                    <div key={idx} className={`flex gap-4 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
                        ...
                        {/* AVATAR */}
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center text-white shrink-0 shadow-md ${msg.role === 'user' ? 'bg-slate-800' : 'bg-blue-600'
                            }`}>
                            {msg.role === 'user'
                        ...
                        {/* MESSAGE */}
                        <div
                            className={`p-4 rounded-2xl text-sm leading-relaxed shadow-sm whitespace-pre-line ${msg.role === 'user'
